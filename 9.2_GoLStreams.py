...
# Bis hierhin alles gleich, aber jetzt: num_concurrent neu fuer Anzahl gleichzeitiger Ausfuehrungen
...
def update_gpu(frameNum, imgs, newLattices_gpu, lattices_gpu, N, streams, num_concurrent):
    for k in range(num_concurrent):
        ...
        # vorher war der [:] PyCuda Slice Operator, der im Prinzip wie der PyCuda set operator funktioniert, hier brauchen wir aber den set_async operator.
...
ani = animation.FuncAnimation(fig, update_gpu,
                                  fargs=(imgs, newLattices_gpu, lattices_gpu, N, streams, num_concurrent), interval=25,
